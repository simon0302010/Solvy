...
def run_inference(image_bytes):  # , output_image_path):
    ...
    for idx, bounding_box in enumerate(result[0]["predictions"]["predictions"]):
        if bounding_box["confidence"] >= 0.1:
            ...
            new_bounding_box["xmin"] = round(
                bounding_box["x"] - (bounding_box["width"] / 2)
            )
            new_bounding_box["ymin"] = round(
                bounding_box["y"] - (bounding_box["height"] / 2)
            )
            new_bounding_box["xmax"] = round(
                bounding_box["x"] + (bounding_box["height"] / 2)
            )
            new_bounding_box["ymax"] = round(
                bounding_box["y"] + (bounding_box["height"] / 2)
            )
    ...
    label_annotator = sv.LabelAnnotator(
        text_position=sv.Position.CENTER_RIGHT, text_scale=0.4, text_padding=2
    )
